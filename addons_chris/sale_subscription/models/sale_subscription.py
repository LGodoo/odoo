...
class SaleSubscription(models.Model):
    ...
    def _prepare_invoice_data(self):
        ...
        next_date_start_years = next_date_month - relativedelta(years=self.recurring_interval,days=-1)  # startdate if years
        ...
        if self.post_billed == True:
            if self.recurring_next_date != datetime.date(self.recurring_next_date.year, self.recurring_next_date.month, last_day_of_month):
                raise UserError(_('Subscriptions app failed to generate a post-billed invoice: %s'
                                  ' Can only generate post-billed invoices if Date of next invoice is set on the last day of a month: "%s".') % (self.name,self.recurring_next_date,))
            else:
                recurringnextdate = next_date_month.replace(day=1)
                self.write({'recurring_next_date': recurringnextdate.replace(day=calendar.monthrange(recurringnextdate.year, recurringnextdate.month)[1])})
                ...
                if self.recurring_rule_type == 'yearly':
                    return {
                        ...
                        'comment': _("This invoice covers the following period (post billed): %s - %s") % (
                    ...
                    # invoice only
                        ...
                        try:
                            ...
                            if subscription.post_billed == True:
                                new_date_plus = new_date.replace(day=calendar.monthrange(new_date.year, new_date.month)[1])
    ...
    def validate_and_send_invoice(self, invoice):
        ...
        email_context = self.env.context.copy()
        invoice.mark_invoice_as_sent = True
