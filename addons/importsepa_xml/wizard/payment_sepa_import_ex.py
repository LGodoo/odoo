...
class PaymentSepaImport(models.TransientModel):
    ...
    def _import_file(self, data_file):
        ...
        for payret_vals in payment_return_raw_list:
                ...
                for i in range(0, len(payret_vals['recordid'])):
                    ...
                    if name:
                        ...
                        for record in payment_ref:
                            if (payret_vals['errorcode'][i]) is not '':
                                ...
                                message = ({
                                             ...
                                             'body': msg,
                                             'subject': "Declined SEPA payment for: " + payret_vals['recordid'][i],
                                             'needaction_partner_ids': [
                                                 (4, self.env['mail.channel'].search([['id', '=', '17']]).id)],
                                             'model': self.env['account.payment'],
                                             'channel_ids': [(4, self.env['mail.channel'].search([['id', '=', '17']]).id)],
                                             'res_id': self.env['mail.channel'].search([['id', '=', '17']]).id,
                                             })

                                record.message_post(type="notification", subtype="mt_note", **message)
                                ...
                                message = ({
                                    ...
                                    'author_id': self.env.user.partner_id.id,
                                    'body': msg,
                                    'subject': "Accepted SEPA payment for: " + payret_vals['recordid'][i],
                                    'needaction_partner_ids': [
                                        (4, self.env['mail.channel'].search([['id', '=', '17']]).id)],
                                    'model': self.env['account.payment'],
                                    'channel_ids': [(4, self.env['mail.channel'].search([['id', '=', '17']]).id)],
            ...
            elif len(payret_vals['status']) > 0 and payret_vals['status'][i] == 'ACCP':
